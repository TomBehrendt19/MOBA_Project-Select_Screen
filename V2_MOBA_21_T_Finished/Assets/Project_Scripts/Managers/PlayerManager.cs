...
public class PlayerManager
{
    ...
    [HideInInspector]
    public GameObject m_Instance;             // A reference to the instance of the Player when it is created.
    [HideInInspector]
	public GameObject m_PlayerGEO;        // The transform that is a parent of all the Player's renderers.  This is deactivated when the Player is dead.
    [HideInInspector]
    ...
    public PlayerMovement m_Movement;        // References to various objects for control during the different game phases.
    ...
    public void Reset()
    {
        ...
        {
			m_Movement.transform.position = m_SpawnPoint.position;
			m_Movement.transform.rotation = m_SpawnPoint.rotation;
        }
    }
}
