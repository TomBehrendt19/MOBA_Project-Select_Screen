...
public class PlayerSetup : NetworkBehaviour 
{
	...
	public override void OnStartClient()
	{
		...
		if (!isServer) //if not hosting, we had the Player to the gamemanger for easy access!
			GameManager.AddPlayer(gameObject, m_PlayerNumber, m_Color, m_PlayerName, m_LocalID);


		// Get all of the renderers of the Player.
		Renderer renderers = m_PlayerRenderers.GetComponent<Renderer>();
		renderers.material.color = m_Color;


		//if (m_PlayerRenderers)
			//m_PlayerRenderers.SetActive(false);
		m_Color.a = 0.3f;
		m_TeamColour.color = m_Color;
